<template>
  <div class="resources-editor flexc">
    <edit-header :is-new="id==='new'" :draft-id="draftId" :acting="updating" :can-delete="!!item" @loadDraft="loadDraft" @saveDraft="saveDraft" @update="updateresource" @delete="deleteresource"/>
    <div class="body">
      <div class="edit flexc">
        ...
        <div class="sub flex">
          ...
          <select class="type" v-model="type">
            ...
            <option v-for="type in resourceTypeList" :key="type.key" :value="type.key">{{ type.name }}</option>
          </select>
        </div>
...
import {resourceList, resourceTypeList} from "~/utils/data";
...
export default {
  ...
  async asyncData({$stickers, params}) {
    ...
    const item = cloneDeep(resourceList.find(v => v.id===id));
    return {
      id,
      draftId: `draft-resource-${id}`,
      item,
      ...
      text: item?(await import(`!!raw-loader!~/rebuild/resources/${id}.md`)).default:'',
  ...
  methods: {
    ...
    async updateresource () {
      ...
      this.updating = true;
      const now = getNow();
      const newId = this.item ? this.id : randomId(resourceList.map(v => v.id));
      if (await createCommit(` update resource:${this.id}`, [processJSON('resource', resourceList, json => {
          const newData = {
            title: this.title,
            link: this.link || '',
            cover: this.cover,
          ...
          path: `rebuild/resources/${newId}.md`,
          content: text
        },{
          path: `rebuild/resources/${newId}.html`,
          content: mdHtml
      }])) {
        notify({
          title: '完成！',
        ...
        this.$router.replace('/resources').then()
      }
      this.updating = false
    },
    async deleteresource () {
      if (this.item && confirm('确认删除?')) {
        this.updating = true;
        if (await createCommit(` delete resource:${this.id}`, [processJSON('resource', resourceList, json => {
            json.splice(json.findIndex(v => v.id === this.id), 1);
          })], [
            {path: `rebuild/resources/${this.id}.html`},
            {path: `rebuild/resources/${this.id}.md`}
        ])) {
          notify({
            title: '完成！',
            type: 'success',
